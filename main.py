...
from config import api_key, api_secret, randomisation_percent
...
def withdraw(coin, address, amount, network=None):
    try:
        ...
        print(result)
        messagebox.showinfo(f"SUCCESS", f"{amount} {coin} successfully withdrawm and sent to {address} {network}")
        return True
    except Exception as e:
        print(e)
        messagebox.showerror(f"ERROR", f"Withdraw failed: {e.message}")
        return False

def add_rand_to_num(num):
    return str(round(random.uniform(num, float(f'1.0{randomisation_percent}')*num), 6))
...
def withdraw_multiple(coin, addresses, amount, delay, rand, network=None ):
    ...
    for address in addresses_list:
        ...
        result = withdraw(coin, address, amount, network=network)
        if result:
            print(f"Success {coin}, {address}, {amount}, {network}")
            print(f" {delay} seconds")
            sleep(delay)
        else:
            print(f"Error {coin}, {address}, {amount}, {network}")
            break
...
def withdraw_gui():
    ...
    rand_check_box = tk.Checkbutton(root, text=f'Rand {randomisation_percent}%',variable=rand_var, onvalue=True, offvalue=False)
